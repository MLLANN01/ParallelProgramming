...
value_t accuracy(label_t* label_test,
    ...
    for (index_t i = 0; i < num_test; i++) {
        ...
        for (index_t k = 0; k < num_classes; k++)
            match = match && (label_test [i  *num_classes+k] ==
...
void NearestNeighbor() {
   ...
   // run parallelized when any command line argument given
    ...
    const int64_t num_entries = 65000;
    const int64_t num_train = 55000;
	...
	//load_binary(input.data(), input.size(), "C:/Users/marsh/OneDrive/Desktop/ImageData/X.bin");
	//load_binary(label.data(), label.size(), "C:/Users/marsh/OneDrive/Desktop/ImageData/Y.bin");
