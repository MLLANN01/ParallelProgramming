...
# pragma omp parallel
	for (int64_t index = 0; index < num_iters; index++) {
...
void softmax() {
	...
	const int64_t num_test = 55000;
	const int64_t num_acc = 10000;
	...
	while (true) {
		...
		TIMERSTOP(training)
			...
			const int64_t off_inp = 55000 * num_features;
		const int64_t off_lbl = 55000 * num_classes;
